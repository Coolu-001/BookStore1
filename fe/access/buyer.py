...
class Buyer:
    ...
    # 收藏图书
    def add_favorite_book(self, book_id) -> int:
        payload = {
            "user_id": self.user_id,
            "book_id": book_id,
        }
        endpoint = urljoin(self.url_prefix, "add_favorite_book")
        headers = {"token": self.token}
        response = requests.post(endpoint, headers=headers, json=payload)
        return response.status_code

    # 获取收藏夹中的图书
    def get_books_favorite(self, user_id) -> int:
        payload = {"user_id": self.user_id}
        endpoint = urljoin(self.url_prefix, "get_books_favorite")
        headers = {"token": self.token}
        response = requests.post(endpoint, headers=headers, json=payload)
        return response.status_code

    # 取消收藏图书
    def remove_favorite_book(self, book_id) -> int:
        payload = {
            "user_id": self.user_id,
            "book_id": book_id,
        }
        endpoint = urljoin(self.url_prefix, "remove_favorite_book")
        headers = {"token": self.token}
        response = requests.post(endpoint, headers=headers, json=payload)
        return response.status_code

    # 收藏商店
    def add_favorite_store(self, store_id) -> int:
        payload = {
            "user_id": self.user_id,
            "store_id": store_id,
        }
        endpoint = urljoin(self.url_prefix, "add_favorite_store")
        headers = {"token": self.token}
        response = requests.post(endpoint, headers=headers, json=payload)
        return response.status_code

    # 获取商店收藏
    def get_stores_favorite(self, user_id) -> int:
        payload = {"user_id": self.user_id}
        endpoint = urljoin(self.url_prefix, "get_stores_favorite")
        headers = {"token": self.token}
        response = requests.post(endpoint, headers=headers, json=payload)
        return response.status_code

    # 取消收藏商店
    def remove_favorite_store(self, store_id) -> int:
        payload = {
            "user_id": self.user_id,
            "store_id": store_id,
        }
        endpoint = urljoin(self.url_prefix, "remove_favorite_store")
        headers = {"token": self.token}
        response = requests.post(endpoint, headers=headers, json=payload)
        return response.status_code