...
class Seller:
    ...
    def create_store(self, store_id):
        ...
        endpoint = urljoin(self.url_prefix, "create_store")
    ...
    def add_book(self, store_id: str, stock_level: int, book_info: book.Book) -> int:
        ...
        # 调用后端接口添加图书
        endpoint = urljoin(self.url_prefix, "add_book")
        headers = {"token": self.token}  
        response = requests.post(endpoint, headers=headers, json=payload)  
        return response.status_code  
    ...
    def add_stock_level(
        ...
        # 调用后端接口增加库存
        endpoint = urljoin(self.url_prefix, "add_stock_level")
        headers = {"token": self.token}  
        response = requests.post(endpoint, headers=headers, json=payload) 
        return response.status_code 
    ...
    def ship_order(self, store_id: str, order_id: str) -> int:
        ...
        # 调用后端接口发货
        endpoint = urljoin(self.url_prefix, "ship_order")
        headers = {"token": self.token} 
        response = requests.post(endpoint, headers=headers, json=payload) 
        return response.status_code 