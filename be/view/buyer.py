...
@bp_buyer.route("/get_books_favorite", methods=["POST"])
def get_books_favorite() -> Tuple[Response, int]:
    try:
        data = request.get_json(silent=True) or {}
        user_id = data.get("user_id", "")
        ...
        buyer_svc = Buyer()
        status_code, result_data = buyer_svc.get_books_favorite(user_id=user_id)
        
        if status_code != 200:
            return _handle_response(status_code, result_data)
        
        return _handle_response(status_code, "获取收藏书籍成功", {"favorite_books": result_data})
        
    except Exception as e:
        return _handle_response(500, "服务器内部错误")

@bp_buyer.route("/add_favorite_book", methods=["POST"])
def add_favorite_book() -> Tuple[Response, int]:
    try:
        data = request.get_json(silent=True) or {}
        user_id = data.get("user_id", "")
        ...
        buyer_svc = Buyer()
        status_code, result_msg = buyer_svc.add_favorite_book(user_id=user_id, book_id=book_id)
        return _handle_response(status_code, result_msg)
        
    except Exception as e:
        return _handle_response(500, "服务器内部错误")

@bp_buyer.route("/remove_favorite_book", methods=["POST"])
def remove_favorite_book() -> Tuple[Response, int]:
    try:
        data = request.get_json(silent=True) or {}
        user_id = data.get("user_id", "")
        book_id = data.get("book_id", "")
        ...
        buyer_svc = Buyer()
        status_code, result_msg = buyer_svc.remove_favorite_book(user_id=user_id, book_id=book_id)
        return _handle_response(status_code, result_msg)
        
    except Exception as e:
        return _handle_response(500, "服务器内部错误")

@bp_buyer.route("/get_stores_favorite", methods=["POST"])
def get_stores_favorite() -> Tuple[Response, int]:
    try:
        data = request.get_json(silent=True) or {}
        user_id = data.get("user_id", "")
        ...
        buyer_svc = Buyer()
        status_code, result_data = buyer_svc.get_stores_favorite(user_id=user_id)
        
        if status_code != 200:
            return _handle_response(status_code, result_data)
        
        return _handle_response(status_code, "获取收藏店铺成功", {"favorite_stores": result_data})
        
    except Exception as e:
        return _handle_response(500, "服务器内部错误")

@bp_buyer.route("/add_favorite_store", methods=["POST"])
def add_favorite_store() -> Tuple[Response, int]:
    try:
        data = request.get_json(silent=True) or {}
        user_id = data.get("user_id", "")
        store_id = data.get("store_id", "")
        ...
        buyer_svc = Buyer()
        status_code, result_msg = buyer_svc.add_favorite_store(user_id=user_id, store_id=store_id)
        return _handle_response(status_code, result_msg)
        
    except Exception as e:
        return _handle_response(500, "服务器内部错误")

@bp_buyer.route("/remove_favorite_store", methods=["POST"])
def remove_favorite_store() -> Tuple[Response, int]:
    try:
        data = request.get_json(silent=True) or {}
        user_id = data.get("user_id", "")
        store_id = data.get("store_id", "")
        ...
        buyer_svc = Buyer()
        status_code, result_msg = buyer_svc.remove_favorite_store(user_id=user_id, store_id=store_id)
        return _handle_response(status_code, result_msg)
