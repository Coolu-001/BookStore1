...
class Buyer(db_conn.DBConn):
    ...
    def add_favorite_book(self, user_id: str, book_id: str) -> Tuple[int, str]:
        try:
            db = self.db
            user_record = db["user"].find_one({"user_id": user_id})
            ...
            if update_result.modified_count == 0:
                return 529, "Failed to update favorites"
    ...
    def remove_favorite_book(self, user_id: str, book_id: str) -> Tuple[int, str]:
        try:
            db = self.db
            user_record = db["user"].find_one({"user_id": user_id})
            ...
            if update_result.modified_count == 0:
                return 529, "Failed to remove book from favorites"
    ...
    def get_books_favorite(self, user_id: str) -> Tuple[int, str]:
        try:
            db = self.db
            user_record = db["user"].find_one({"user_id": user_id})
        ...
        if not favorite_books:
            return 200, "empty favorites"
        else:
            return 200, favorite_books


    def add_favorite_store(self, user_id: str, store_id: str) -> Tuple[int, str]:
        try:
            db = self.db
            user_record = db["user"].find_one({"user_id": user_id})
            ...
            favorite_stores = user_record.get("favorite_stores", [])
            if store_id in favorite_stores:
                return 200, "store already in favorites"

            update_result = db["user"].update_one(
                {"user_id": user_id},
                {"$addToSet": {"favorite_stores": store_id}}
            )
            
            if update_result.modified_count == 0:
                return 529, "Failed to update store favorites"
    ...
    def remove_favorite_store(self, user_id: str, store_id: str) -> Tuple[int, str]:
        try:
            db = self.db
            user_record = db["user"].find_one({"user_id": user_id})
            ...
            if store_id not in favorite_stores:
                return 200, "store not in favorites"
            
            update_result = db["user"].update_one(
                {"user_id": user_id},
                {"$pull": {"favorite_stores": store_id}}
            )

            if update_result.modified_count == 0:
                return 529, "Failed to remove store from favorites"
    ...
    def get_stores_favorite(self, user_id: str) -> Tuple[int, str]:
        try:
            db = self.db
            user_record = db["user"].find_one({"user_id": user_id})
        ...
        if not favorite_stores:
            return 200, "empty store favorites"
        else:
            return 200, favorite_stores